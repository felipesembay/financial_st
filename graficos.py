...
def grafico():
    ...
    with f3:
        selecao = st.selectbox("Status", ["Realizado", "Previsão"])
    with f4:
        st.write("") # Espaço em branco para alinhar os filtros

    # Consultando o banco de dados MySQL
    query_receita = "SELECT DATE_FORMAT(Data, '%Y-%m') AS mes_ano, SUM(Valor) AS Total FROM receitas WHERE Data BETWEEN %s AND %s AND Status = %s GROUP BY mes_ano"
    query_despesa = "SELECT DATE_FORMAT(Data, '%Y-%m') AS mes_ano, SUM(Valor) AS Total FROM despesas WHERE Data BETWEEN %s AND %s AND Status = %s GROUP BY mes_ano"
    ...
    GROUP BY mes_ano
    """
    # Parâmetros para as queries de receita e despesa
    params_receita_despesa = (data_inicial.strftime('%Y-%m-%d'), data_final.strftime('%Y-%m-%d'), selecao)

    # Parâmetros para as queries de investimento
    params_investimento = (data_inicial.strftime('%Y-%m-%d'), data_final.strftime('%Y-%m-%d'))

    # Executando as queries
    receitas = pd.read_sql_query(query_receita, cnx, params=params_receita_despesa)
    despesas = pd.read_sql_query(query_despesa, cnx, params=params_receita_despesa)
    investimentos_compra = pd.read_sql(investimento_compra_query, cnx, params=params_investimento)
    investimentos_venda = pd.read_sql(investimento_venda_query, cnx, params=params_investimento)
    ...
    query_receita_cat = "SELECT Categoria, SUM(Valor) as total FROM receitas WHERE Data BETWEEN %s AND %s AND Status = %s GROUP BY Categoria"
    query_despesa_cat= "SELECT Categoria, SUM(Valor) as total FROM despesas WHERE Data BETWEEN %s AND %s AND Status = %s GROUP BY Categoria"
    params = (data_inicial.strftime('%Y-%m-%d'), data_final.strftime('%Y-%m-%d'), selecao)
